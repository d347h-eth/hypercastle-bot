...
-   Remote rate limiting: a dedicated controller in `TwitterPublisher` owns all X rate state for posting. It parses headers (preferring `userDay` daily caps), persists state in `meta`, clamps values, and enforces a small reserve so we never use the 17th slot. When a stored reset elapses, it restores remaining to the limit so `/me` probing is unnecessary. If `remaining` is empty it throws `RateLimitExceededError(reset/remaining/limit)` and defers; 429 handling updates state from headers and exponential retry is capped to `x-rate-limit-reset` with a small buffer after reset. If headers are missing, it sets a synthetic reset using per-slot spacing anchored to the last spent call so the system can self-heal without new requests. Media uploads capture `media_uploaded_at` and force re-upload after 24h to avoid expired media IDs.
