...
describe("BotService recovery", () => {
    it("marks stale posting as posted when matching tweet exists", async () => {
        ...
        expect(repo.markPosted).toHaveBeenCalledWith("s1", "t1", expectedText, expect.any(Number));
    ...
    it("requeues stale posting when tweet not found", async () => {
        ...
        expect(repo.requeueStale).toHaveBeenCalledWith("s1", expect.any(Number));
    });
    ...
    it("handles multiple items: recovers found ones and requeues missing ones", async () => {
        ...
        expect(repo.markPosted).toHaveBeenCalledWith("s1", "t1", s1Text, expect.any(Number));
        expect(repo.requeueStale).toHaveBeenCalledWith("s2", expect.any(Number));
    ...
    it("aborts recovery if publisher fails to fetch timeline", async () => {
        ...
        publisher.fetchRecent = vi.fn().mockRejectedValue(new Error("API Error"));
