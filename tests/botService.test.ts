...
describe("BotService", () => {
    ...
    it("posts queued sales on poll", async () => {
        ...
        vi.mocked(publisher.checkRateLimit).mockResolvedValue({
            limit: 17,
            remaining: 10,
        });
        ...
        expect(workflow.process).toHaveBeenCalledWith({
            sale: s1,
            attemptCount: 0,
        });
    ...
    it("defers on remote rate limit (low remaining)", async () => {
        ...
        vi.mocked(feed.fetchRecent).mockResolvedValue([]);
        const s1 = makeSale("s1");
        vi.mocked(repo.claimNextReady).mockReturnValue({
            sale: s1,
            attemptCount: 0,
        });
        ...
        expect(repo.requeueAfterRateLimit).toHaveBeenCalledWith(
            "s1",
            expect.any(Number),
        );
    });
    ...
    it("handles workflow rate limit error", async () => {
        ...
        vi.mocked(workflow.process).mockRejectedValue(
            new RateLimitExceededError("limit", 12345, 0, 17),
        );
